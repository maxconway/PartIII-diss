...
for(j in 1:nrow(runs)){
  ...
  measureat = seq(1,max(w$generation),2)
  volume=sapply(measureat,function(i){
    hypervolumeMonteCarlo(w[w$generation==i,c('biomass','maxsyn','minsyn')],100)
  })
...
colnames(chromosomes)<-c((names),c('?','?','?','?','?','?'),c('maxsyn','minsyn','biomass','front','crowding'))#assignments don't hit
chromosomes$totalknockouts=GDMO[GDMO$id==id&GDMO$generation==500,'knockouts']
image(1:500,y=1:212,as.matrix(chromosomes[order(chromosomes$biomass,chromosomes$maxsyn),1:212]),col=c('ghostwhite','red'))
@
One major advantage of GDMO is the information it can give not only on the 
