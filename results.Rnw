...
<<methods_GDMO_subsets,echo=FALSE,cache=TRUE,dependson=c('dominated','ResultsSetup','results_subset_setup')>>=
...
pc<-princomp(GDMO.S.chromosomes[,c('nmaxsyn','nminsyn','nbiomass')])
GDMO.S.chromosomes$pos<-pc$scores[,1]
pc<-princomp(GDMO.M.chromosomes[,c('nmaxsyn','nminsyn','nbiomass')])
...
Since the Geobacter Metallireducens model appears to be, at least in some ways, actually superior to the G. Sulfurreducens model, the analysis of knockout patterns here continues from the. \todo{phrase nicely}

First, we view the global knockout pattern to detect any global features.

<<geoMknockouts, fig.cap='knockouts in G. Sulfurreducens'>>=
heatmap(x=as.matrix(GDMO.S.chromosomes[,1:212]*1),
        Rowv=GDMO.S.chromosomes$pos,
        Colv=cor(GDMO.S.chromosomes[1,1:212]),
        ylab='genes',
