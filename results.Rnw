...
\chapter{Results}
This chapter details the results of the investigation: first the technical evaluation focusses on the properties of GDMO and GDLS themselves, then the biological evaluation discusses the synthesis possible with both Geobacter models, the knockout strategies that were used, and the results of using GDMO for evaluating potential knockins from E. Coli.
\todo{section references}

\section{Technical Evaluation}
This section focusses on the relative merits of GDMO and GDLS, and why GDMO was chosen for further use.
First the convergence behaviour of both algorithms is evaluated, to ensure that the comparison is as fair as possible; secondly the solutions from both technques are compared.
...
<<GDMOevals,results='asis',dependson='ResultsSetup'>>=
...
Figure~\ref{fig:convergence} shows hypervolume against generation for a number of runs of GDMO. This shows that convergence occured quickly---within \num{200} generations. 
...
Ultimately approach~\ref{enum:approachtaken} was taken, since this is effective without requiring modification to the core algoritm. At this stage this was used to select fronts for further analysis---for each strain the Pareto front was found for the union of:
\begin{enumerate}
...
<<comparison,echo=F,fig.cap='G. Sulfurreducens Pareto front, overlaid with GDLS results',out.width='0.4\\textheight',dependson='GDMO_fronts'>>=
...
To avoid repetition the G. Metallireducens section focusses more strongly on knockout patterns since although the patterns in both species were investigated extensively, these patterns may be more biologically relevant.
...
Figure~\ref{fig:sulfurreducensmin} shows a pronounced jump at around 0.935 in the biomass axis. This is discussed further in Section~\ref{sec:gskink}. \label{sec:gskinkfind}

<<sulfurreducensmax,echo=F,out.width='0.4\\textheight',dependson='GDMO_fronts',fig.cap='Pareto front of maximum reduction potential for Geobacter Sulfurreducens, normalized against wild type'>>=
plot(nmaxsyn~nbiomass,hs,xlab='biomass',ylab='reduction potential',col='red')
@

<<sulfurreducensmin,echo=F,out.width='0.4\\textheight',fig.cap='Pareto front of minimum reduction potential for Geobacter Sulfurreducens, normalized against wild type',dependson='GDMO_fronts'>>=
plot(nminsyn~nbiomass,ls,xlab='biomass',ylab='reduction potential',col='red')
@
...
Figure~\ref{fig:sulfurreducensmin} \label{sec:gskink} shows a `knee' in the biomass vs minimum synthesis Pareto front of G. Sulfurreducens. \todo{find these in chart}
To look at this in more detail, we can extract the relevant sets of strains and compare their knockouts.
<<kinkanalysis, out.width='0.4\\textheight', dependson=c('GDMO_S','GDMO_fronts')>>=
...
<<geoSknockouts, echo=F, out.width='0.4\\textheight', fig.cap='knockouts in G. Sulfurreducens',dependson=c('GDMO_S')>>=
...
<<commonmodifications, echo=F, out.width='0.4\\textheight', fig.cap='Common knockouts in G. Sulfurreducens',dependson=c('k2')>>=
plot(k2,xlab='modification index',ylab='usages') 
abline(v=10)
@

Figure~\ref{fig:commonmodifications} shows that 10 knockouts (to the left of the line) are highly common. These may be of particular interest, and the associated reactions are detailed in table~\ref{tab:GDMO_S_overrepresented}. \todo{make some comment}

<<GDMO_S_overrepresented,echo=F,dependson=c('GDMO_S'),results='asis'>>= 
print(xtable(
  GDMO.S.reactions[GDMO.S.reactions$GeneAssociation %in% names(head(k2,10)),c('SubSystem','Equation')],
...
<<m_fronts,echo=F,out.width='0.4\\textheight',fig.cap='G. Metallireducens Pareto Front'>>=
require('lattice',quietly=TRUE)
cloud(GDMO.M$nbiomass~GDMO.M$nmaxsyn*GDMO.M$nminsyn)
@
The Pareto front for G. Metallireducens electricity production shown in figure~\ref{fig:m_fronts} is appears to be almost completely linear, which was to start with extremely suprising, and suggested that some error had been made---one does not expect that random knockouts would be able to achieve such a precise tradeoff. To verify this shape, a number of comparisons were made with the G. Sulfurreducens model.
<<straight_line_verification1,fig.pos='p',fig.cap='verification of G. Metallireducens Pareto Front',fig.pos='p',dependson=c('GDMO_M','GDMO_S')>>=
...
<<straight_line_verification2,fig.cap='PCA of Pareto Fronts: proportion of variance explained by each component'>>=
...
<<minsynMpattern, echo=F, out.width='0.4\\textheight', dependson=c('GDMO_M'),fig.cap='A large penalty in minimum synthesis occurs for very high values of maximum synthesis'>>=
plot(nminsyn~nmaxsyn,GDMO.M,ylab='normalized minimum synthesis',xlab='normalized maximum synthesis',col='blue')
@
...
<<geoMknockouts, echo=F, out.width='0.4\\textheight', fig.pos='h', dependson=c('GDMO_M_chromosomes2'), fig.cap='knockouts (blue) in G. Metallireducens'>>=
...
The reactions associated with these genes presented in table~\ref{tab:GDMO_M_essential}\todo{check this appears correctly}, however, since an essential gene list is already known~\cite{Sun2009}, these reactions were considered to be very bad candidates for increasing electricity producing, and so removed.
...
<<geoMknockoutsnonessential, echo=F, out.width='0.4\\textheight', fig.pos='h', dependson=c('GDMO_M_nonessential'), fig.cap='knockouts (blue) in G. Metallireducens, nonessential reactions only'>>=
...
These were taken to be those whos presence in a strain is strongly related to the position of that strain in the first principle component of its objectives.
<<geoMKOtrendslinear, out.width='0.4\\textheight', fig.pos='h', dependson=c('GDMO_M_nonessential'),fig.scap='Linearly smoothed knockouts',fig.cap='knockouts for each geneset in the Pareto front for G. Metallireducens, smoothed by a linear model. Larger x-axis numbers have larger biomass production'>>=
plot(NULL,NULL,
...
<<geoMKOtrendsloess, out.width='0.4\\textheight', fig.pos='h', dependson=c('GDMO_M_nonessential'),fig.scap='Polynomial smoothed knockouts',fig.cap='knockouts for each geneset in the Pareto front for G. Metallireducens, smoothed by a polynomial model'>>=
plot(NULL,NULL,
     xlab='1st principle component of objectives',
     ylab='knockouts',
     xlim=range(GDMO.M.nonessential$pos),
...
Specifically, the orange box in figure~\ref{fig:geoMKOtrendsloess} shows a number of knockouts of particular interest. 
<<GDMO_M_popular, echo=F ,dependson=c('GDMO_S','geoMKOtrendsloess'),results='asis'>>= 
print(xtable(
  GDMO.M.reactions[GDMO.M.reactions$GeneAssociation %in% as.character(interesting),c('SubSystem','Equation')],
