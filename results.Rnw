...
<<ResultsSetup,autodep=TRUE,echo=FALSE>>=
#sources
...
runs=aggregate(GDMO[,c('strain','pop','generation','id')],by=list(GDMO$id),max)
runs$population=runs$pop

#GDLS
data(GDLS)
GDLS.runs=unique(GDLS[,c('strain','id','nbhdsz','M')])
GDLS<-merge(GDLS,natural)
# normalize objectives
GDLS$nbiomass <- GDLS$biomass/GDLS$natbiomass
GDLS$nmaxsyn <- GDLS$maxsyn/GDLS$natmaxsyn
GDLS$nminsyn <- GDLS$minsyn/GDLS$natminsyn
@
...
<<convergence,fig.cap='Convergence of evolution runs',cache=TRUE>>=
...
for(j in 1:nrow(runs)){
  ...
  volume=sapply(measureat,function(i){
    hypervolumeMonteCarlo(w[w$generation==i,c('nbiomass','nmaxsyn','nminsyn')],2) # must be > 1
  })
...
\end{enumerate}
<<fronts,echo=FALSE,eval=FALSE>>=
#unused
sulfurreducens<-with(GDMO,GDMO[strain=='sulfurreducens'&((pop==500&generation%in%c(400,450,500))
                                                         |
                                                           (pop==1000&generation==250)]))
sulfurreducens<-sulfurreducens[!dominated(sulfurreducens[,c('nmaxsyn','nbiomass','nminsyn')])]
@
...
Figure~\ref{fig:sulfurreducensmax} shows a roughly linear tradeoff between maximum reduction potential and biomass generation is possible, up to a maximum of a \SI{\Sexpr{(max(hs$nmaxsyn))*100}}{\percent} of wild type yield, at the expense of \SI{\Sexpr{min(hs$nbiomass)}}{\percent} biomass production.
...
However, only a small increase over wild type of \SI{\Sexpr{(max(ls$nminsyn))*100}}{\percent} was possible while maintaining viability.
