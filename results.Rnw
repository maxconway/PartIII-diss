...
<<convergence,fig.cap='Convergence of evolution runs',cache=TRUE,dependson=c('hypervolume','ResultsSetup')>>=
...
\end{enumerate}
<<methods_GDMO_sulfurreducens,eval=TRUE,cache=TRUE,dependson=c('dominated','ResultsSetup')>>=
#unused
#GDMO.sulfurreducens<-with(GDMO,GDMO[strain=='sulfurreducens'&((pop==500&generation%in%c(400,450,500))
#                                                              |(pop==1000&generation==250))])
#GDMO.sulfurreducens<-GDMO.sulfurreducens[!dominated(GDMO.sulfurreducens[,c('nmaxsyn','nbiomass','nminsyn')]),]
GDMO.sulfurreducens=GDMO[GDMO$strain=='sulfurreducens'&GDMO$pop==500&GDMO$generation==500,]
@

<<methods_GDMO_subsets,echo=FALSE,cache=TRUE,dependson=c('dominated','ResultsSetup','methods_GDMO_sulfurreducens')>>=
#extract pareto fronts for each dimension
s<-GDMO.sulfurreducens

hs<-s[,c('nbiomass','nmaxsyn')]
hs<-hs[!dominated(hs),]

ls<-s[,c('nbiomass','nminsyn')]
ls<-ls[!dominated(ls),]
@
...
<<comparison,dependson='methods_GDMO_subsets'>>=
...
<<echo=FALSE,dependson='methods_GDMO_subsets'>>=
...
<<results_GDMO_sulf_knockouts,echo=FALSE,dependson=c('ResultsSetup','methods_GDMO_sulfurreducens')>>=
id=with(GDMO.runs,unique(GDMO.runs[strain=='sulfurreducens'&pop==500&generation==500,'id']))
...
sulfurreducens.reactions=read.delim('./data/geo_s_react.txt',)
@
...
<<geoSknockouts, fig.cap='knockouts in G. Sulfurreducens'>>=
heatmap(x=as.matrix(chromosomes[,1:212]*1),
        Rowv=chromosomes$pos,
        Colv=cor(chromosomes[1,1:212]),
        ylab='genes',
        xlab='strain'
)
@
...
<<kinkanalysis,dependson=c('emthods_GDMO_subsets','results_GDMO_sulf_knockouts')>>=
A <- merge(chromosomes,ls) # A is all strains in the minsyn vs biomass pareto front
...
<<dependson=c('results_GDMO_sulf_knockouts')>>=
sulfurreducens.reactions[sulfurreducens.reactions$GeneAssociation %in% names(k1),c('OfficialName','Equation')]
@
...
<<commonmodifications,fig.cap='Common modifications',>>=
...
<<>>= 
sulfurreducens.reactions[sulfurreducens.reactions$GeneAssociation %in% names(head(k2,10)),c('OfficialName','Equation')]
@
\todo{plot knockouts onto genome indicies}

One downside of simply counting knockouts is that it does not account for epistasis, which might be caused by multiple knockouts being needed to force the use of a particular pathway.
<<s.epistasis>>=
...
for(strain in c('sulfurreducens','metallireducens')){
  ...
  plot(knockouts~generation,b,main=unique(strain),sub='knockouts vs time')
  plot(nbiomass~generation,b,ylim=c(0,1),sub='biomass vs time')
  plot(nmaxsyn~generation,b,ylim=c(1,1.05),sub='maximum synthesis vs time')
  plot(nminsyn~generation,b,ylim=c(0,1.05),sub='minimum synthesis vs time')
#  cloud(nbiomass~generation*nmaxsyn,sub='maximum synthesis vs biomass vs time')
}
@
...
These extra resources cannot necessarily be used outside of specific pathways, and so they produce extraneous features in the pareto front where they can, or cannot, be used.
