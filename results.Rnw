...
<<minsynMpattern,fig.cap='A large penalty in minimum synthesis occurs for very high values of maximum synthesis'>>=
...
<<geoMknockouts, fig.cap='knockouts (blue) in G. Metallireducens'>>=
...
<<geoMKOtrendslinear,fig.scap='Linearly smoothed knockouts',fig.cap='knockouts for each geneset in the pareto front for G. Metallireducens, smoothed by a linear model. Larger x-axis numbers have larger biomass production>>=
plot(NULL,NULL,
...
<<geoMKOtrendsloess,fig.scap='Polynomial smoothed knockouts',fig.cap='knockouts for each geneset in the pareto front for G. Metallireducens, smoothed by a polynomial model'>>=
plot(NULL,NULL,
     xlab='1st principle component of objectives',
     ylab='knockouts',
     xlim=range(GDMO.M.nonessential$pos),
