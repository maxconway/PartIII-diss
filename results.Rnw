...
\end{enumerate}
<<GDMO_S,eval=TRUE,cache=TRUE,dependson=c('dominated','ResultsSetup')>>=
...
GDMO.S.chromosomes<-GDMO.S[GDMO.S.nondom,]
...
@
<<GDMO_M,eval=TRUE,cache=TRUE,dependson=c('dominated','ResultsSetup')>>=
...
GDMO.M.chromosomes<-GDMO.M[GDMO.M.nondom,]
...
And, from these fronts, the pareto fronts were calculated for pairs of objectives.

<<methods_GDMO_subsets,cache=TRUE,dependson=c('dominated','ResultsSetup','GDMO_S','GDMO_M')>>=
...
\subsection{GDLS}
The knockout strategies returned by GDLS provided a maximum synthesis of \sexpr{GDLS$nmaxsyn*100}\si{\percent}. This was possible using a small neighbourhoodsize (nbhdsz) of 3 and branching factor (M) of 1.
GDLS was not able to improve upon this even with parameters that allowed it to explore a much larger region.
<<GDLS_solutionquality>>=
cloud(nmaxsyn~nbhdsz+M,GDLS,default.scales=list(arrows=F),col='red')
@
...
@
We can see that the solutions produced by GDLS are in the Pareto front, but that GDLS is not able to achieve as high a synthesis rate as GDMO.
This is likely to be because the wild type is in a local minimum for \ce{FE2} production, and since GDLS is a local, rather than global, search strategy, it cannot escape this minimum. 
This theory is supported by the `knee' in the pareto front that GDLs does not appear to have been able to pass.
...
However, GDMO created a solution that was superior to any provided by GDLS in only \sexpr{min(GDMO[GDMO$strain=='sulfurreducens'&GDMO$nmaxsyn>1.014673,'cputime'])}\si{\seconds} of cputime, at one point.

Since GDMO provided appeared to provide better knockout vectors more quickly than GDLS, and with the added benefit of describing the whole Pareto front, GDMO was chosen for use from this point forwards.
...
<<geoSknockouts, out.width='0.8\\linewidth', fig.cap='knockouts in G. Sulfurreducens',dependson=c('GDMO_S')>>=
heatmap(x=as.matrix(GDMO.S.chromosomes[,grep(pattern='GSU.*',colnames(y.chromosomes))]*1),
        Rowv=GDMO.S.chromosomes$pos,
        Colv=cor(GDMO.S.chromosomes[,grep(pattern='GSU.*',colnames(y.chromosomes))]),
        ylab='genes',
        xlab='strain'
)
@
...
<<kinkanalysis, out.width='0.8\\linewidth', dependson=c('GDMO_S','methods_GDMO_subsets')>>=
...
heatmap(x=as.matrix(A[,grep(pattern='GSU.*',colnames(y.chromosomes))]*1),
        Rowv=NA,
        Colv=NULL,
        labRow=A$nminsyn,
        labCol=colnames(A[,grep(pattern='GSU.*',colnames(y.chromosomes))]),
...
<<commonmodifications, out.width='0.8\\linewidth', fig.cap='Common modifications',dependson=c('k2')>>=
...
for(a in list(GDMO.S, GDMO.M)){
  colour <- switch(unique(a$strain),
                   sulfurreducens='red',
                   metallireducens='blue')
  b<-a[sample(nrow(a),500),]
  plot(knockouts~generation,b,main=unique(a$strain),sub='knockouts vs time',pch='.',col=colour)
...
<<GDMO_M_chromosomes2,dependson=c('GDMO_S','GDMO_M','GDMO_S','GDMO_M')>>=
