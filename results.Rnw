...
source('./data/hypervolumeMonteCarlo.R')
plot(NULL,NULL,xlab='generation',ylab='hypervolume','l',ylim=c(0,1.1),xlim=c(0,500))
legend('bottomright',paste(runs$strain,' pop=',runs$pop,' gen=',runs$gen),col=rainbow(nrow(runs)),pch='l',cex=0.5)
for(j in 1:nrow(runs)){
  ...
  lines(measureat,volume/max(volume),col=rainbow(nrow(runs))[j])
}
@
