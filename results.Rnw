...
Note that much larger increases in hypervolume were possible for E. Coli in Acetate production, so that this could be not presented on the same scale.
...
<<GDLS_solutionquality,echo=F,fig.cap='Solutions found by GDLS. Maximum synthesis achieved (normalized against wildtype) against the two parameters tested.',dependson=c('ResultsSetup')>>=
data(GDLS)
...
<<straight_line_verification1,fig.pos='p',out.width='\\textwidth', out.height='\\textheight', fig.cap='Verification of Geobacter Metallireducens Pareto Front', fig.height=10, fig.width=5, fig.pos='p',dependson=c('GDMO_M','GDMO_S')>>=
...
\subsection{Geobacter Metallireducens Knockins}
Finally, having studied knockouts in Geobacter Sulfurreducens and Metallireducens, potential knockins were studied.

This was achieved by adding to the G. Metallireducens metabolic model a number of reactions from the iJO1366 E. Coli model. These started off turned off, so that the algorithm would turn them on if they were useful.

The reactions from E. Coli were those in three subsystems:
\begin{itemize}
...
The resulting set of strains had a Pareto front between their objectives that displayed no significant difference from pure Geobacter Metalirreducens. 
This is not hugely surprising, and shows that knockin studies require a much more targeted approach.
This is likely to be because Geobacter already has the capability to efficiently produce biomass from the resources available, so that E. Coli genes cannot help with that, and E. Coli does not specialize in reduction of inorganic ions, so Geobacter is likely to be more thoroughly equipped for this task.
