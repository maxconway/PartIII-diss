...
<<sulfurreducensmax,dependson='methods_GDMO_subsets',fig.cap='Pareto front of maximum reduction potential for Geobacter Sulfurreducens, normalized against wild type'>>=
plot(nmaxsyn~nbiomass,hs,xlab='biomass',ylab='reduction potential')
@

<<sulfurreducensmin,fig.cap='Pareto front of minimum reduction potential for Geobacter Sulfurreducens, normalized against wild type',dependson='methods_GDMO_subsets'>>=
...
<<kinkanalysis,dependson=c('emthods_GDMO_subsets','results_GDMO_loadChromosomes')>>=
...
<<minsynMpattern,,dependson=c('GDMO_M'),fig.cap='A large penalty in minimum synthesis occurs for very high values of maximum synthesis'>>=
...
<<geoMknockouts,dependson=c('GDMO_M_chromosomes2'), fig.cap='knockouts (blue) in G. Metallireducens'>>=
...
This forms a larger proportion of the genes visible here than in Sulfurreducens, which may be partly due to the supplementation strategy in Sulfurreducens, but could also be due to differences in the performance of the network reduction algorithm used, and so may not have biological relevance.
...
<<geoMKOtrendslinear,dependson=c('GDMO_M_nonessential'),fig.scap='Linearly smoothed knockouts',fig.cap='knockouts for each geneset in the pareto front for G. Metallireducens, smoothed by a linear model. Larger x-axis numbers have larger biomass production'>>=
plot(NULL,NULL,
...
for(k in grep('Gmet_.*',names(GDMO.M.nonessential),value=TRUE)){
  ...
  lines(smoothed~pos,interaction,col='navy')
}
@
...
<<geoMKOtrendsloess,dependson=c('GDMO_M_nonessential'),fig.scap='Polynomial smoothed knockouts',fig.cap='knockouts for each geneset in the pareto front for G. Metallireducens, smoothed by a polynomial model'>>=
plot(NULL,NULL,
     xlab='1st principle component of objectives',
     ylab='knockouts',
     xlim=range(GDMO.M.nonessential$pos),
...
for(k in grep('Gmet_.*',names(GDMO.M.nonessential),value=TRUE)){
  ...
  interaction$smoothed<-predict(interaction.fit)
  lines(smoothed~pos,interaction,type='l',col='navy')
}
@
