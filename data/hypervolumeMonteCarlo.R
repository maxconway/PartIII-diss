hypervolumeMonteCarlo <- function(x,num){
  ...
  #stochastic portion
  points=data.frame(nbiomass=runif(num,min(x$nbiomass),max(x$nbiomass)),nmaxsyn=runif(num,min(x$nmaxsyn),max(x$nmaxsyn)),nminsyn=runif(num,min(x$nminsyn),max(x$nminsyn)))
