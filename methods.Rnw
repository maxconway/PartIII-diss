...
Described in 2009 in~\ref{Mahadevan2006}, this model focusses on the \ce{FE3} reducing capabilities of Geobacter Sulfurreducens. 
...
As discussed in section~\ref{sex:complexity}, this results in much slower analysis of these networks.
...
\item[Gurobi] is a commerical toolkit also with a good number of interfaces on all relevant platforms. The literature suggests that it is the fastest performing toolkit~\ref{Meindl2012}.
\end{description}
...
\lable{hypervolume}
Techniques to compare the output of multi-objective optimization algorithms are an area of active research~\ref{Okabe2003}. 
...
One relatively common metric for comparison of pareto fronts is the hypervolume enclosed by the front~\ref{Zitzler1999,Zitzler}. 
