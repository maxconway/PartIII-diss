...
\label{sec:NSGA}
NSGA-II~\ref{Deb2002a} is a multi-objective evolutionary algorithm that provides a pareto front by using two criteria to decide the quality of a solution:
\begin{description}
...
Genetic Design by Branch and Bound~\ref{Egen2012} is an enhancment on GDLS that can provide a large increase in speed by using a Branch and Bound strategy.
...
Genetic Design by Multi-objective Optimization~\ref{Costanza2012} is a genetic design algorithm based on the multi-objective evolutionary optimization algorithm NSGA-II, as described in Section~\ref{sec:NSGA}.
