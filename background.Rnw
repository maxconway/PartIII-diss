...
Furthermore, the ability to utilize insoluble materials as electron acceptors make Geobacter interesting as a candidate for use in bacterial fuel cells with, and without, membranes, and the pilli used to connect to transport electrons have applications in themselves, as nanowires.
...
FBA~\cite{Orth2010} seeks to determine the flux vector at a given time by using the stoichiometric matrix to define a network, with the fluxes forming the edges which must be determined. 
...
A solution \(A\) Pareto dominates \(B\) if \(A\) is better than \(B\) in terms of every objective ways. 
\(A\) is Pareto optimal if no solution Pareto dominates it. 

The notation \(A \succ B\) will be used to mean \(A\) Pareto dominates \(B\).
...
The non-dominated set is termed the Pareto Front, or Pareto Surface, and describes the shape of the tradeoff between multiple objectives, which can provide insight over and above what could be provided by a single solution. 
In the context of Genetic design, as used in Genetic Design by Multi-objective Optimization~\ref{sec:GDMO}, multiple solutions allow us to investigate how knckouts interact with each other, in a way which is not possible with a single solution.
...
\begin{description}
\item[Rank] Solutions are split in to groups by rank. The familiar, nondominated Pareto front is assigned rank 1, while for all other solutions p, \(\mathrm{rank}(p) = 1 + \max( \mathrm{rank}(q) | q \succ p) )\), where \( a \succ b \) means a dominates b.
...
The search space is still far too large to manually characterize, but fortunately, and these metabolome level effects can fortunately also be well modelled by FBA, which works well for large scale, steady state effects such as overproduction, but cannot model dynamic, reactive systems. 
Since FBA presents a useful computational metabolic model, especially when annotated with gene-protein-reaction data, it allows the development of algorithms that can search part of the huge space of possible knockout strategies (for instance around \num{5.3e210} in the E. Coli models used here), in order to draft strategies for in-vitro evaluation.
...
For this reason, GDLS was used as a proxy for GDBB in comparing between algorithms, and ultimately solution quality, rather than the time taken, was found to be the deciding factor.
